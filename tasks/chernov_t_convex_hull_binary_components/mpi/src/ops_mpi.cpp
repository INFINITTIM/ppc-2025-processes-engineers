...
void ChernovTConvexHullBinaryComponentsMPI::FindConnectedComponentsMpi() {
  ...
  for (int local_row = 0; local_row < local_rows; ++local_row) {
    std::copy_n(
        local_pixels_.begin() + static_cast<std::size_t>(local_row) * static_cast<std::size_t>(width_), width_,
        extended_pixels.begin() + static_cast<std::size_t>(offset + local_row) * static_cast<std::size_t>(width_));
  }

  ExchangeBoundaryRows();

  int global_y_offset = start_row_ - (has_top ? 1 : 0);
  std::vector<std::vector<bool>> visited_extended(static_cast<std::size_t>(extended_rows),
                                                  std::vector<bool>(static_cast<std::size_t>(width_), false));
  std::vector<std::vector<std::pair<int, int>>> all_components;

  ProcessExtendedRegion(global_y_offset, extended_rows, extended_pixels, visited_extended, all_components);
...
void ChernovTConvexHullBinaryComponentsMPI::ProcessExtendedRegion(
    int global_y_offset, int extended_rows, const std::vector<int> &extended_pixels,
    std::vector<std::vector<bool>> &visited_extended, std::vector<std::vector<std::pair<int, int>>> &all_components) {
  ...
  for (int ey = 0; ey < extended_rows; ++ey) {
    for (int col = 0; col < width_; ++col) {
      ...
      if (extended_pixels[pixel_idx] == 1 &&
          !visited_extended[static_cast<std::size_t>(ey)][static_cast<std::size_t>(col)]) {
        std::vector<std::pair<int, int>> comp;
        std::queue<std::pair<int, int>> q;
        q.emplace(col, ey);
        visited_extended[static_cast<std::size_t>(ey)][static_cast<std::size_t>(col)] = true;
          ...
          for (int dir = 0; dir < 4; ++dir) {
            ...
            if (nx >= 0 && nx < width_ && ny >= 0 && ny < extended_rows) {
              ...
              if (extended_pixels[neighbor_idx] == 1 &&
                  !visited_extended[static_cast<std::size_t>(ny)][static_cast<std::size_t>(nx)]) {
                visited_extended[static_cast<std::size_t>(ny)][static_cast<std::size_t>(nx)] = true;
                q.emplace(nx, ny);
              }
            }
          }
...
void ChernovTConvexHullBinaryComponentsMPI::ReceiveHullsFromProcess(int src, int count) {
  std::vector<int> sizes(count);
  MPI_Recv(sizes.data(), count, MPI_INT, src, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  int pts = 0;
  MPI_Recv(&pts, 1, MPI_INT, src, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  if (pts > 0) {
    ...
    for (int sz : sizes) {
      std::vector<std::pair<int, int>> hull(sz);
      for (int j = 0; j < sz; ++j, idx += 2) {
        hull[j] = {pts_data[idx], pts_data[idx + 1]};
      }
